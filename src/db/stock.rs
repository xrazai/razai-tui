...
pub async fn list_estoque_saldos(pool: &PgPool) -> Result<Vec<EstoqueSaldoRecord>, sqlx::Error> {
    let rows = sqlx::query(
        ...
        WHERE tc.ativo = TRUE
        GROUP BY tc.tecido_id, tc.cor_id, tc.sku, t.nome, c.nome, tc.preco_atacado_override, tc.preco_varejo_override, t.preco_atacado, t.preco_varejo
        UNION ALL
        SELECT
            te.tecido_id,
        ...
        WHERE te.ativo = TRUE
        GROUP BY te.tecido_id, te.estampa_id, te.sku, t.nome, e.nome, te.preco_atacado_override, te.preco_varejo_override, t.preco_atacado, t.preco_varejo
...
pub async fn list_estoque_ordens(pool: &PgPool) -> Result<Vec<EstoqueOrdemRecord>, sqlx::Error> {
    let rows = sqlx::query(
        ...
        SELECT
            eo.id,
            to_char(eo.created_at AT TIME ZONE 'America/Sao_Paulo', 'DD/MM/YYYY HH24:MI') AS created_at,
            eo.tecido_id,
            eo.item_id,
            eo.usa_estampas,
            ...
            eo.quantidade::float8 AS quantidade,
            eo.status,
            eo.fornecedor_id,
...
pub async fn reset_sale_stock_movements(
    ...
    for item in itens {
        ...
        let faltante = if saldo_antes <= 0.0 {
            ...
            (item.quantidade - saldo_antes).max(0.0)
        };
        if faltante > 0.0 {
            sqlx::query(
                r#"
